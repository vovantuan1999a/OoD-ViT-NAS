...
from .utils_D.data_loaders_imgnet_id import ImageNetDLoader
...
def build_dataset(is_train, args, cfg_dataset=None):
    transform = build_transform(is_train, args)
    if str(cfg_dataset['data_set']) == 'ImageNet-C':
        root = os.path.join(os.path.join(cfg_dataset['imagenet_root_dir'],cfg_dataset['type_corruption'].replace(" ","_")), cfg_dataset['level_corruption'])
        dataset = datasets.ImageFolder(root, transform=transform)
        nb_classes = 1000
    elif cfg_dataset['data_set'] == 'ImageNet-P':
        root = os.path.join(cfg_dataset['imagenet_root_dir'],cfg_dataset['type_corruption'].replace(" ","_"))
        dataset = datasets.ImageFolder(root, transform=transform)
        nb_classes = 1000
    elif cfg_dataset['data_set'] == 'ImageNet-D':
        imagenet_d_dir = cfg_dataset['imagenet_root_dir']
        dataset = dict()
        subset_list = [cfg_dataset['type_corruption']] 
        for subset in subset_list:
            test_loader = torch.utils.data.DataLoader(
                ImageNetDLoader(os.path.join(imagenet_d_dir, subset), transform=transform),
                batch_size=100, shuffle=True,
                num_workers=10, pin_memory=True)
            dataset[subset] = test_loader
        nb_classes =1000

    elif cfg_dataset['data_set'] == 'ImageNet-O':
        PATH_TO_IMAGENET_O = cfg_dataset['imagenet_root_dir']
        PATH_TO_IMAGENET_VAL = cfg_dataset['imagetnet_original_dir']

        noes = dset.ImageFolder(root=PATH_TO_IMAGENET_O, transform=transform)
        ...
        val_examples_imagenet_o = dset.ImageFolder(root=PATH_TO_IMAGENET_VAL, transform=transform)
        val_loader_imagenet_o = torch.utils.data.DataLoader(val_examples_imagenet_o, batch_size=128, shuffle=False,
                                                num_workers=4, pin_memory=True)
        nb_classes = 1000
        dataset = (val_loader_imagenet_o, noe_loader)                                    
    else:
        root = cfg_dataset['imagenet_root_dir']
        dataset = datasets.ImageFolder(root, transform=transform)
        nb_classes = 1000
    return dataset, nb_classes


def build_dataset_transform(is_train, args, folder_name=None):
    transform = build_transform(is_train, args)
