...
def convert_100_to_1k(target,output):
    n_target = []
    n_output = []
    output = output.detach().cpu().numpy()
    for i,cl in enumerate(target):
        inx = str(cl.item())
        
        if name_class1k[inx] not in name_class100:
            continue
        n_target.append(cl)
        n_output.append(output[i].tolist())
    n_target = torch.tensor(n_target,dtype=torch.int32)
    n_output = torch.as_tensor(n_output, dtype=torch.float32)

    return n_target,n_output
...
@torch.no_grad()
def get_predictions(loader,device, amp, net=None, mask=None):
    confidence = []
    correct = []
    num_correct = 0
    with torch.no_grad():
        for data, target in loader:
            ...
            if amp:
                with torch.cuda.amp.autocast():
                    # output = model_module(images)
                    output = net(data)[:,imagenet_o_mask]
                    # loss = criterion(output, target)
            else:
                # output = model_module(images)
                output = net(data)[:,imagenet_o_mask]
...
@torch.no_grad()
def evaluate_O(in_loader,out_loader, model, device, amp=True, choices=None, mode='super', retrain_config=None):
    criterion = torch.nn.CrossEntropyLoss()
    ...
    with torch.no_grad():
        ...
        confidence_in, correct, num_correct = get_predictions(in_loader,device,amp, net=model_module, mask=None)
        in_score = -confidence_in
        confidence_out, correct_out, num_correct_out = get_predictions(out_loader,device,amp, net=model_module, mask=None)
        ...
        aurra = calibration_tools.print_measures_old(aurocs, auprs, fprs, method_name='MSP')
...
@torch.no_grad()
def evaluate_D(data_loader, model, device, amp=True, choices=None, mode='super', retrain_config=None):
    criterion = torch.nn.CrossEntropyLoss()
    ...
    with torch.no_grad():
        ...
        # for sample, example in metric_logger.log_every(data_loader, 10, header):
        for datatype, test_loader in data_loader.items():
            ...
            for sample, example in tqdm(enumerate(test_loader), total=len(test_loader), desc=pbar_prefix):
            # for sample, example in metric_logger.log_every(test_loader, 10, header):
                images = example['images']
                target = example['labels']
                # print
                # print(images)
                ...
                cnt += batch_size
                acc1 = top1_cnt * 100.0 / cnt
                acc5 = top5_cnt * 100.0 / cnt
...
@torch.no_grad()
def evaluate_OOD(data_loader, model, device, amp=True, choices=None, mode='super', retrain_config=None,names=None):
    criterion = torch.nn.CrossEntropyLoss()

    # image100_class = data_loader.classes
    # print(names)
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    # dict_class_image1k = {}
    # list_data = data_loader.classes
    # switch to evaluation mode
    with torch.no_grad():
        model.eval()
        if mode == 'super':
        ...
        for images, target in metric_logger.log_every(data_loader, 10, header):
            ...
            # n_target,n_images = convert_100_to_1k(target,images)
            # print(n_images.shape)
            # if int(n_images.shape[0]) == 0:
                # continue
            # n_target = n_target.to(device, non_blocking=True)
            # n_images = n_images.to(device, non_blocking=True)
            # print(target)
            # convert_100_to_1k(target,names,name_class)
            # print("maxxx",torch.max(target))
            # for i in range(target.shape[0]):
            #     index_name = target[i].item()
               
            #     dict_class_image1k[cl] = index_name
            ...
            if amp:
                ...
                # loss = criterion(output, target)
            # print(output.shape)
            # n_target,n_output = convert_100_to_1k(target,output)
            # n_target = n_target.to(device, non_blocking=True)
            # n_output = n_output.to(device, non_blocking=True)
            acc1, acc5 = accuracy(output, target, topk=(1, 5))
